...
- Last updated: 2026-09-08 (D-4/E-1/E-2 execution landed — re-validate + sequential stop-on-failure + report; D57. Prior: D-1/D56, C-2/D55, D-2/D54)
...
| D57 | **Execution service (`execution/service.py`) = confirm re-validation (D-4) + sequential stop-on-failure (E-1) + reporting (E-2).** On confirm it **re-validates against fresh state (D19)**; a *material change* — defined narrowly as **re-validation flipping to invalid** — returns `REVALIDATE` (re-show) with nothing submitted. A now-closed market **holds** (D17). Otherwise it submits orders one at a time in the plan's sells-first order and **stops on the first failure** (a reject or an Alpaca outage), leaving the rest in cash (D15); success is *accepted* granularity (D16). Reports completed N of M, the failure reason, and **best-effort cash remaining** (⚠️ pre-settlement — fills are async, D16). Each submission + response is recorded (D20). ⚠️ Flagged: re-planning on price drift (recomputing targets at confirm) is deferred — v1's material-change test is validity-flip only. | Mostly specified by D15/D16/D19; the reviewable calls are the narrow material-change definition and the pre-settlement cash figure — both surfaced in QUESTIONS.md. Maps to the H-2 fake's accept/reject/fail scripting for deterministic tests. |
| D56 | **D-1 proposal assembly (`proposal/service.py`)** orchestrates the whole read-side pipeline (parse → basis → map → constrain/plan → validate → allocate → market clock) into a discriminated `ProposalOutcome`: `PROPOSAL` / `CLARIFY` / `REFUSE` / `UNAVAILABLE` / `ERROR`, so nothing ambiguous or invalid reaches confirm (D4/D10/D18). **Restatement** = the parser's `summary` + B-2 basis notes + B-3 mapping + applied constraints — **no extra LLM call** (default, flagged in QUESTIONS.md; surfaces basis D2 + mapping D5 as required). A **closed market** is still a `PROPOSAL` carrying a warning (D-3/D17). **Alpaca-unreachable at propose time → `UNAVAILABLE`** ("try again") — this is A-5's propose-side behavior landing inside D-1 (per D48). When an `AuditStore` is supplied, the request, LLM calls, and proposal are persisted (D20); F-1 completes the full end-to-end record. Confirm-time re-validation (D-4) and execution (E) consume the proposal separately. | Assembling the pipeline is mostly wiring; the reviewable call is restatement-from-summary vs a fresh LLM call (chose no extra call — cheaper/deterministic, flagged). Realizes D-3 and A-5's propose-side in one place. |
| D55 | **C-2 constraint-solving (`planning/constraints.py`) — default readings** (2026-09-08, ⚠️ defaults flagged in QUESTIONS.md). `cash_floor` → reduce the **investable base** to `equity − floor` (a rebalance then leaves exactly the floor in cash). `exclude_asset` → **set the holding aside**: never sold, value removed from the investable base so the rest rebalances around it (exclusion target treated as a **symbol**; a category exclusion is not resolved to symbols in v1). `only_new_deposits` → **unsupported in v1** (deposits aren't tracked) → refuse+explain. Unsatisfiable numerics (floor ≥ equity, nothing left to invest) → refuse. No partial orders on refusal (D10). Applied constraints returned for confirm display (D8). Integrated into the planner via a new `plan_from_state(investable_equity, protected)` seam (C-1 behavior unchanged when absent). | These are the natural readings ("keep $5k and rebalance the rest", "leave AAPL alone"); the reduce-investable vs refuse choice and category-exclusion support are the reviewable calls — surfaced in QUESTIONS.md, not silently locked. |
| D54 | **D-2 order validation (`planning/validation.py`) — buying power counts expected sell proceeds.** (2026-09-08, ⚠️ flagged for review.) Because execution is sells-first (D15), a plan is affordable when total buy cost ≤ `buying_power` **+ expected sell proceeds**; otherwise self-funding rebalances would falsely fail. **Simplification:** sell proceeds are treated as immediately available; settlement (cash-account T+1) and margin are **not modeled** in v1 (the H-2 fake doesn't simulate settlement either) — real-sandbox unsettled-funds rejections surface via the typed reject path at execution (E), not at validation. Other rules: notional orders require a fractionable asset; a fractional `qty` on a non-fractionable asset is rejected; a sell `qty` may not exceed the held position (no shorting in v1); notional ≥ $1; symbol must be tradable/known. Invalid → reject-with-reason, nothing auto-adjusted, no partial proposal (D18/D10). | Counting sell proceeds is correctness-determined (else rebalancing never validates); the settlement/margin simplification is the reviewable call — surfaced rather than silently chosen. |
